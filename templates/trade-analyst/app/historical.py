"""Interface for historical OHLC (Open, High, Low, Close) data with Eastern Time support"""
...
from .utils.timeutils import now_eastern, eastern_to_utc, utc_to_eastern, format_timestamp, EASTERN_TIMEZONE
from .middleware.timezone_middleware import schwab_api_timezone_handler, timezone_aware, eastern_time_required

logger = logging.getLogger(__name__)


class HistoricalInterface:
    """Interface for retrieving historical OHLC data with Eastern Time handling"""
    ...
    async def get_ohlc(self, 
        ...
        """
        Get historical OHLC data for a symbol with Eastern Time timestamps
        
        Args:
            symbol: Stock symbol (e.g., 'AAPL', 'MSFT')
            start_date: Start date for historical data (Eastern Time)
            end_date: End date for historical data (Eastern Time)
            interval: Data interval ('1m', '5m', '15m', '1h', '1D', '1W', '1M')
            
        Returns:
            List of OHLC records with Eastern Time timestamps or None if error
        """
        try:
            logger.info(f"Fetching OHLC data for {symbol} from {format_timestamp(start_date, 'eastern')} to {format_timestamp(end_date, 'eastern')}, interval: {interval}")
    ...
    async def get_latest_ohlc(self, symbol: str, interval: str = '1D', count: int = 10) -> Optional[List[Dict[str, Any]]]:
        """
        Get latest OHLC records for a symbol with Eastern Time
        ...
        Returns:
            List of latest OHLC records with Eastern Time timestamps
        """
        try:
            logger.info(f"Fetching latest {count} OHLC records for {symbol}")
            
            # Calculate date range for latest records using Eastern Time
    ...
    def _generate_synthetic_ohlc(self, 
                                symbol: str, 
                                start_date: datetime, 
                                end_date: datetime, 
                                interval: str) -> List[Dict[str, Any]]:
        """
        Generate synthetic OHLC data for testing with Eastern Time
        ...
        Returns:
            List of synthetic OHLC records with Eastern Time timestamps
        """
        import random
        ...
        while current_date <= end_date:
            ...
            # Generate synthetic OHLC values
            ...
            # Create Eastern Time timestamp
            if interval == '1D':
                # Use market close time for daily data
                timestamp = EASTERN_TIMEZONE.localize(
                    datetime.combine(current_date.date(), datetime.strptime("16:00:00", "%H:%M:%S").time())
                )
            else:
            ...
            synthetic_data.append(ohlc_record)
        ...
        logger.info(f"Generated {len(synthetic_data)} synthetic OHLC records for {symbol}")
