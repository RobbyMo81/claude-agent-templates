"""Command Line Interface for Trade Analyst Application with Eastern Time support"""
...
from .middleware.timezone_middleware import timezone_middleware
...
def serve_callback(ctx, host, port, debug):
    """Start the callback server for OAuth authentication with Eastern Time logging"""
    current_time = now_eastern()
    logger.info(f"Starting callback server on {host}:{port} at {format_timestamp(current_time, 'eastern')}")
    logger.info(f"Server timezone: {EASTERN_TIMEZONE}")
    
    # Initialize timezone middleware
    timezone_middleware.init_app(flask_app)
