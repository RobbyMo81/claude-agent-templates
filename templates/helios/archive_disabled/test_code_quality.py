...
class CodeQualityTester:
    ...
    def add_issue(self, file_path: str, line_num: int, issue_type: str, description: str, severity: str = "warning"):
    ...
    def test_python_files(self) -> List[Dict[str, Any]]:
        ...
        for file_path in python_files:
            try:
                ...
                self.add_issue(str(file_path), 0, "file_error", f"Error analyzing file: {e}", "error")
        
        return self.issues
    
    def test_typescript_files(self) -> List[Dict[str, Any]]:
        """Test all TypeScript/JavaScript files for quality issues"""
        ts_files = list(self.project_root.rglob("*.ts")) + list(self.project_root.rglob("*.tsx")) + list(self.project_root.rglob("*.js"))
    ...
    def analyze_python_file(self, file_path: Path):
        ...
        try:
            ...
            self.add_issue(str(file_path), e.lineno or 0, "syntax_error", f"Syntax error: {e.msg}", "error")
    ...
    def analyze_python_line(self, file_path: Path, line_num: int, line: str):
        ...
        if stripped.startswith('def ') and not stripped.endswith(':'):
            if '(' in stripped and ')' not in stripped:
                self.add_issue(str(file_path), line_num, "truncated_function", "Function definition appears truncated - missing closing parenthesis")
        
        # Check for truncated class definitions
        if stripped.startswith('class ') and not stripped.endswith(':'):
            if '(' in stripped and ')' not in stripped:
                self.add_issue(str(file_path), line_num, "truncated_class", "Class definition appears truncated")
        
        # Check for incomplete import statements
        if stripped.startswith('from ') and ' import ' not in stripped:
            self.add_issue(str(file_path), line_num, "incomplete_import", "Import statement appears incomplete")
        
        # Check for incomplete string literals
        quote_count = stripped.count('"') + stripped.count("'")
        if quote_count % 2 != 0 and not stripped.startswith('#'):
            self.add_issue(str(file_path), line_num, "unmatched_quotes", "Unmatched quotes detected")
        
        # Check for lines ending with commas (potential truncation)
        if stripped.endswith(',') and len(stripped) > 100:
            self.add_issue(str(file_path), line_num, "suspicious_comma", "Long line ending with comma - possible truncation")
        
        # Check for incomplete dictionary/list definitions
        if (stripped.count('{') != stripped.count('}')) or (stripped.count('[') != stripped.count(']')):
            if not any(x in stripped for x in ['"""', "'''", '#']):  # Ignore multiline strings and comments
                self.add_issue(str(file_path), line_num, "unmatched_brackets", "Unmatched brackets detected")
        ...
        if '(' in stripped and ')' not in stripped and not stripped.endswith('\\'):
            if not stripped.startswith('#') and 'def ' not in stripped and 'class ' not in stripped:
                self.add_issue(str(file_path), line_num, "incomplete_call", "Potentially incomplete function call")
    ...
    def analyze_typescript_line(self, file_path: Path, line_num: int, line: str):
        ...
        if 'function' in stripped and '(' in stripped and ')' not in stripped:
            self.add_issue(str(file_path), line_num, "truncated_function", "Function definition appears truncated")
        
        # Check for incomplete interface definitions
        if stripped.startswith('interface ') and '{' not in stripped:
            self.add_issue(str(file_path), line_num, "incomplete_interface", "Interface definition appears incomplete")
        
        # Check for incomplete import statements
        if stripped.startswith('import ') and 'from' in stripped and not stripped.endswith(';'):
            if "'" not in stripped[-10:] and '"' not in stripped[-10:]:
                self.add_issue(str(file_path), line_num, "incomplete_import", "Import statement may be incomplete")
        ...
        if '<' in stripped and '>' in stripped:
            ...
            if open_tags - close_tags - self_closing > 0:
                self.add_issue(str(file_path), line_num, "unmatched_jsx", "Potentially unmatched JSX tags")
        
        # Check for incomplete object literals
        if stripped.count('{') != stripped.count('}') and not stripped.startswith('//'):
            self.add_issue(str(file_path), line_num, "unmatched_braces", "Unmatched braces detected")
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if stripped.startswith('def '):
                ...
                if not stripped.endswith(':'):
                    self.add_issue(str(file_path), line_num, "incomplete_function_def", "Function definition missing colon")
                ...
                # Check for common function ending patterns
                    ...
                    if in_function:
                        self.add_issue(str(file_path), function_start, "incomplete_function", "Function may be incomplete - reaches end of file")
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if stripped.startswith('from ') and ' import ' in stripped:
                ...
                if stripped.endswith('import'):
                    self.add_issue(str(file_path), line_num, "incomplete_import", "Import statement ends with 'import' keyword")
                ...
                if stripped.startswith('from .') or stripped.startswith('from ..'):
                    ...
                    if module_part.count('.') > 2:
                        self.add_issue(str(file_path), line_num, "complex_relative_import", "Complex relative import detected")
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            for char_pos, char in enumerate(line):
                ...
                # Handle closing brackets
                    ...
                    if not stack:
                        self.add_issue(str(file_path), line_num, "unmatched_closing", f"Unmatched closing bracket '{char}' at position {char_pos}")
                    else:
                        opening, _, _ = stack.pop()
                        if pairs[opening] != char:
                            self.add_issue(str(file_path), line_num, "mismatched_brackets", f"Mismatched bracket pair at line {line_num}")
        
        # Check for unclosed brackets
        for opening, line_num, pos in stack:
            self.add_issue(str(file_path), line_num, "unclosed_bracket", f"Unclosed bracket '{opening}' at position {pos}")
    ...
    def check_component_completeness(self, file_path: Path, content: str):
        ...
        component_pattern = re.compile(r'const\s+(\w+):\s*React\.FC.*=.*\(\w*\)\s*=>\s*{')
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if match:
                ...
                has_return = any('return' in l for l in remaining_lines[:20])  # Check next 20 lines
                
                if not has_return:
                    self.add_issue(str(file_path), line_num, "incomplete_component", f"Component '{component_name}' may be missing return statement")
