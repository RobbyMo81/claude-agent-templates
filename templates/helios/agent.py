...
class PowerballNet(nn.Module):
    ...
    def __init__(self,
        ...
        # Embedding layers
        self.white_ball_embedding = nn.Embedding(self.white_ball_vocab + 1, hidden_dim // 2)
        self.powerball_embedding = nn.Embedding(self.powerball_vocab + 1, hidden_dim // 4)
    ...
    def forward(self, white_balls: torch.Tensor, powerball: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        ...
        white_embedded = self.white_ball_embedding(white_balls).sum(dim=2)  # (batch, seq, hidden//2)
        
        # Embed powerball
        powerball_embedded = self.powerball_embedding(powerball.squeeze(-1))  # (batch, seq, hidden//4)
        
        # Combine embeddings
        # Pad powerball embedding to match white ball embedding size
        powerball_padded = F.pad(powerball_embedded, (0, self.hidden_dim // 4))
        combined = torch.cat([white_embedded, powerball_padded], dim=-1)  # (batch, seq, hidden)
    ...
    def predict_next_draw(self, white_balls: torch.Tensor, powerball: torch.Tensor) -> Dict[str, Any]:
        """
        ...
        with torch.no_grad():
            ...
            predictions = {
                'white_balls': {
                    'numbers': (white_top5.indices + 1).cpu().numpy().tolist(),  # +1 for 1-based indexing
...
class MLPowerballAgent:
    ...
    def prepare_data(self, historical_data: pd.DataFrame) -> Tuple[Tuple[torch.Tensor, torch.Tensor], Tuple[torch.Tensor, torch.Tensor]]:
        """
        ...
        """
        logger.info(f"🔍 DEBUG PREPARE_DATA: Starting with {len(historical_data)} historical draws")
        logger.info(f"🔍 DEBUG PREPARE_DATA: Historical data columns: {list(historical_data.columns)}")
        logger.info(f"🔍 DEBUG PREPARE_DATA: Historical data shape: {historical_data.shape}")
        
        # Extract white balls and powerball
        white_balls = historical_data[['white_ball_1', 'white_ball_2', 'white_ball_3', 
                                     'white_ball_4', 'white_ball_5']].values
        powerball = historical_data['powerball'].values
        
        logger.info(f"🔍 DEBUG PREPARE_DATA: white_balls shape: {white_balls.shape}")
        logger.info(f"🔍 DEBUG PREPARE_DATA: powerball shape: {powerball.shape}")
        logger.info(f"🔍 DEBUG PREPARE_DATA: white_balls sample: {white_balls[:3]}")
        logger.info(f"🔍 DEBUG PREPARE_DATA: powerball sample: {powerball[:3]}")
        ...
        logger.info(f"🔍 DEBUG PREPARE_DATA: Created {len(sequences_white)} training sequences")
        
        # Convert to tensors
        X_white = torch.tensor(np.array(sequences_white), dtype=torch.long).to(self.device)
        X_powerball = torch.tensor(np.array(sequences_powerball), dtype=torch.long).unsqueeze(-1).to(self.device)
        y_white = torch.tensor(np.array(targets_white), dtype=torch.long).to(self.device)
        y_powerball = torch.tensor(np.array(targets_powerball), dtype=torch.long).to(self.device)
        
        logger.info(f"🔍 DEBUG PREPARE_DATA: Final tensor shapes:")
        logger.info(f"   X_white: {X_white.shape}")
        logger.info(f"   X_powerball: {X_powerball.shape}")
        logger.info(f"   y_white: {y_white.shape}")
        logger.info(f"   y_powerball: {y_powerball.shape}")
        
        return (X_white, X_powerball), (y_white, y_powerball)
    
    def train(self, historical_data: pd.DataFrame, epochs: int = 100, learning_rate: float = 0.001):
        """
        ...
        # Prepare data
        (X_white, X_powerball), (y_white, y_powerball) = self.prepare_data(historical_data)
        ...
        for epoch in range(epochs):
            ...
            # Debug tensor shapes
            logger.info(f"🔍 DEBUG AGENT - Tensor shapes in training loop:")
            logger.info(f"   outputs['white_balls']: {outputs['white_balls'].shape}")
            logger.info(f"   y_white: {y_white.shape}")
            logger.info(f"   y_white[:, 0]: {y_white[:, 0].shape}")
            
            # Calculate losses for each ball position
            white_losses = []
            for i in range(5):  # 5 white balls
                logger.info(f"🔍 DEBUG AGENT - Ball position {i}:")
                logger.info(f"   outputs['white_balls'].shape: {outputs['white_balls'].shape}")
                logger.info(f"   y_white[:, {i}].shape: {y_white[:, i].shape}")
                logger.info(f"   outputs['white_balls'].dtype: {outputs['white_balls'].dtype}")
                logger.info(f"   y_white[:, {i}].dtype: {y_white[:, i].dtype}")
                logger.info(f"   y_white[:, {i}] min/max: {y_white[:, i].min()}/{y_white[:, i].max()}")
                logger.info(f"   outputs['white_balls'][0, :5]: {outputs['white_balls'][0, :5]}")
                logger.info(f"   y_white[:, {i}][:5]: {y_white[:, i][:5]}")
            ...
            if filename.endswith('.pth'):
                ...
                if os.path.exists(metadata_path):
                    try:
                        ...
                        logger.warning(f"Could not load metadata for {model_name}: {str(e)}")
