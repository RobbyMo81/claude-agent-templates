...
class TestPhase4IntegrationSimplified(unittest.TestCase):
        ...
        with memory_store._get_connection() as conn:
            ...
            test_sessions = [
                ...
                ('job_gpt35_001', 'gpt-3.5-turbo', 'completed', 
                 ...
                 100, '{"learning_rate": 0.001, "batch_size": 32}', None, current_time, current_time),
                ('job_gpt35_002', 'gpt-3.5-turbo', 'completed',
                ...
                ('job_gpt4_001', 'gpt-4', 'completed',
                 ...
                 100, '{"learning_rate": 0.0005, "batch_size": 16}', None, current_time, current_time),
                ('job_gpt4_002', 'gpt-4', 'completed',
                ...
                ('job_claude_001', 'claude-3', 'completed',
                 ...
                 100, '{"learning_rate": 0.002, "batch_size": 24}', None, current_time, current_time),
            ]
    ...
    def test_02_direct_analytics_engine(self):
        ...
        best_model = comparison.performance_ranking[0][0] if comparison.performance_ranking else "unknown"
        print(f" Model comparison: best_model={best_model}")
        
        # Test ensemble recommendations
        recommendations = analytics.generate_ensemble_recommendations(['gpt-3.5-turbo', 'gpt-4', 'claude-3'])
    ...
    def test_03_api_endpoints_via_test_client(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f" Performance endpoint returned {response.status_code}")
            ...
            if response.status_code == 200:
                ...
                print(f" Matrix endpoint returned {response.status_code}")
            
            # Test comparison endpoint
            response = self.client.post('/api/analytics/comparison', 
                                      json={'models': ['gpt-3.5-turbo', 'gpt-4']})
            if response.status_code == 200:
                data = response.get_json()
                self.assertIn('best_model', data)
                print(f" Comparison endpoint: best={data['best_model']}")
            else:
                print(f" Comparison endpoint returned {response.status_code}")
    ...
    def test_04_end_to_end_analytics_workflow(self):
        ...
        for model in models:
            try:
                ...
                print(f"  {model}: loss={perf.final_loss:.3f}, efficiency={perf.efficiency_score:.2f}")
        ...
        if len(models) >= 2:
            ...
            for i, rec in enumerate(recommendations[:2]):  # Show first 2
                print(f"    {i+1}. {rec.ensemble_type}: {rec.models} (confidence: {rec.confidence:.2f})")
        
        # Step 5: Get performance matrix
        matrix = analytics.get_performance_matrix()
        print(f"  Performance matrix: {len(matrix.models)} models, {len(matrix.metrics)} metrics")
        
        # Step 6: Get trend analysis
        trends = analytics.get_trend_analysis(days=7)
        print(f"  Trend analysis: {len(trends.trends)} data points over {trends.period}")
...
def run_simplified_integration_tests():
    ...
    if success_rate >= 90:
        ...
        print("  Phase 4 implementation needs debugging")
