...
class FocusedCodeQualityTester:
    ...
    def analyze_project_files(self):
        ...
        for file_path in project_files:
            try:
                ...
                self.add_issue(str(file_path), 0, "file_error", f"Error analyzing: {e}", "error")
    ...
    def check_python_line_issues(self, file_path: Path, line_num: int, line: str):
        ...
        if stripped.startswith('def '):
            if not stripped.endswith(':'):
                self.add_issue(str(file_path), line_num, "truncated_function_def", 
                             "Function definition missing colon", "error")
            ...
            if '(' in stripped:
                ...
                if open_count != close_count:
                    self.add_issue(str(file_path), line_num, "unmatched_function_parens", 
                                 "Unmatched parentheses in function definition", "error")
        
        # 2. Check for truncated class definitions
        if stripped.startswith('class '):
            if not stripped.endswith(':'):
                self.add_issue(str(file_path), line_num, "truncated_class_def", 
                             "Class definition missing colon", "error")
        
        # 3. Check for incomplete import statements
        if stripped.startswith('from ') and ' import ' not in stripped and not stripped.endswith('\\'):
            self.add_issue(str(file_path), line_num, "incomplete_import", 
        ...
        if len(line) > 200 and not line.endswith(('\\', ',', ')', '}', ']')):
            ...
            self.add_issue(str(file_path), line_num, "possible_truncation", 
    ...
    def check_python_function_completeness(self, file_path: Path, lines: List[str]):
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            # Check if function ends without proper body
                ...
                if not has_body:
                    self.add_issue(str(file_path), function_start, "empty_function", 
    ...
    def check_frontend_line_issues(self, file_path: Path, line_num: int, line: str):
        ...
        if stripped.startswith('interface ') and '{' not in stripped and not stripped.endswith('{'):
            self.add_issue(str(file_path), line_num, "incomplete_interface", 
        ...
        if '<' in stripped and '>' in stripped:
            ...
            if open_tags > (close_tags + self_closing):
                self.add_issue(str(file_path), line_num, "unclosed_jsx", 
                             "JSX tags may be unclosed or truncated")
        
        # 5. Check for very long lines that might be truncated
        if len(line) > 300 and not any(line.strip().endswith(x) for x in [',', ';', '{', '}', '(', ')']):
            self.add_issue(str(file_path), line_num, "possible_truncation", 
    ...
    def check_component_completeness(self, file_path: Path, lines: List[str]):
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if re.search(r'const\s+\w+:\s*React\.FC', line):
                ...
                has_return = False
                for check_line in lines[line_num:line_num + 50]:  # Check next 50 lines
                    if 'return' in check_line:
                        has_return = True
                        break
                
                if not has_return:
                    self.add_issue(str(file_path), component_start, "missing_component_return", 
